...
const getUserReceived = `-- name: GetUserReceived :many
...
func (q *Queries) GetUserReceived(ctx context.Context, arg GetUserReceivedParams) ([]Transfer, error) {
	rows, err := q.db.QueryContext(ctx, getUserReceived, arg.ToAccountID, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const getUserSent = `-- name: GetUserSent :many
SELECT id, from_account_id, to_account_id, amount, created_at
FROM transfers
WHERE
    from_account_id = $1
...
func (q *Queries) GetUserSent(ctx context.Context, arg GetUserSentParams) ([]Transfer, error) {
	rows, err := q.db.QueryContext(ctx, getUserSent, arg.FromAccountID, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const listTransferHistory = `-- name: ListTransferHistory :many
SELECT id, from_account_id, to_account_id, amount, created_at
FROM transfers
WHERE
    from_account_id = $1
...
func (q *Queries) ListTransferHistory(ctx context.Context, arg ListTransferHistoryParams) ([]Transfer, error) {
	rows, err := q.db.QueryContext(ctx, listTransferHistory, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
